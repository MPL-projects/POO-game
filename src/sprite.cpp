...
// Time delta for sprite updates
double delta_time_ = 1.0 / 150.0;

// Constants for spritesheet rows corresponding to directions
int const SPRITESHEET_UP = 5;
int const SPRITESHEET_LEFT = 4;
int const SPRITESHEET_RIGHT = 4;
int const SPRITESHEET_DOWN = 3;

// Maps to associate directions with their corresponding spritesheet rows, movements, and flip states
map<Direction, int> row_sprite = {
    {Direction::UP, 5}, 
    {Direction::LEFT, 4}, 
    {Direction::RIGHT, 4}, 
    {Direction::DOWN, 3}
...
map<Direction, vector<double>> dm = {
    {Direction::UP, {0, -(500.0 * delta_time_)}}, 
    {Direction::LEFT, {-(500.0 * delta_time_), 0}}, 
    {Direction::RIGHT, {(500.0 * delta_time_), 0}}, 
    {Direction::DOWN, {0, (500.0 * delta_time_)}}
};

map<Direction, SDL_RendererFlip> fl = {
    {Direction::UP, SDL_FLIP_NONE}, 
    {Direction::LEFT, SDL_FLIP_HORIZONTAL}, 
    {Direction::RIGHT, SDL_FLIP_NONE}, 
    {Direction::DOWN, SDL_FLIP_NONE}
};

// Constructor for the Sprite class
Sprite::Sprite(const char* path_to_sprite, float ratio_) : m_spritesheet(path_to_sprite, 10, 6), ratio(ratio_) {
    // Initialize renderer and position
    ...
    // Initialize coordinates and bounding box offsets
    x = m_position.x;
    y = m_position.y;
...
void Sprite::update() {
    ...
    if (alive) {
        if (block) {
            col = 0;
            off = 3;
            dir = (m_direction == Direction::NONE) ? m_direction_prev : m_direction;
        } else {
...
void bounds(double &p, int bb_p, int bb_s, int bb_off_p, int size_p) {
    if (bb_p < 0) {
        p = -bb_off_p;
    } else if (bb_p + bb_s > size_p) {
        p = size_p - bb_s - bb_off_p;
    }
}
...
// Update the bounding boxes based on the sprite's position
...
// Get the bounding boxes of the sprite
std::vector<SDL_Rect>& Sprite::get_boxes() {
    return bb;
}

// Change the sprite's skin
